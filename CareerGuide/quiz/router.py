...
def questions_page(request: Request, user_check = Depends(get_current_user)):
...
def type_a_page(request: Request, user_check = Depends(get_current_user)):
    # Проверяем, является ли результат страницей ошибки
    if not isinstance(user_check, int):
        return user_check
...
def type_b_page(request: Request, user_check = Depends(get_current_user)):
    # Проверяем, является ли результат страницей ошибки
    if not isinstance(user_check, int):
        return user_check
...
def type_c_page(request: Request, user_check = Depends(get_current_user)):
    # Проверяем, является ли результат страницей ошибки
    if not isinstance(user_check, int):
        return user_check
...
def type_d_page(request: Request, user_check = Depends(get_current_user)):
    # Проверяем, является ли результат страницей ошибки
    if not isinstance(user_check, int):
        return user_check
...
@router.post('/clear_quiz_progress')
async def clear_quiz_progress_endpoint(
    request: Request,
    user_check = Depends(get_current_user)
):
    # Проверяем, является ли результат страницей ошибки
